...
function formatContent($arrData, $type, $random = false){
	switch ($type){
		case 'webinar': $messageTitle = "Webinar: "; $messageLink = 'Register Now'; $arrType = array('Webinar'); $class='webinar'; break;
		case 'ondemand webinar': $messageTitle = "On-Demand Webinar: "; $messageLink = 'Watch Now'; $arrType = array('On-Demand Webinar'); $class='ondemand'; break;
		case 'whitepaper': $messageTitle = "Whitepaper: "; $messageLink = 'Read Now'; $arrType = array('WP', 'E-Book'); $class='whitepaper'; break;
		case 'podcast': $messageTitle = "Podcast: "; $messageLink = 'Listen Now'; $arrType = array('Podcast'); $class='podcast'; break;
	}
	for ($i = 0; $i < count($arrData); $i++){
		$message .= '<li class="'.$class.'">'.$arrData[$i]['title'];
		if ($type == 'webinar'){
		...
		$message .= ' <a href="#" onclick="registerAfter(`'.$arrData[$i]['form_id'].'`,`'.$arrData[$i]['url'].'`,'.strval($i).',`'.$class.'`);">'.$messageLink.'</a>'.getSponsor($arrData[$i]['campaign'], $arrType);
		$message .= '</li>';
	}
	return $message;
}
...
function getWebinarsRaw($id, $maxArticles, $db){
	...
	for ($i = 0; $i < count($arrArticleMeta); $i++){
		...
		if ($numOccurrencesMeta == 0)
			$numOccurrencesMeta = 1;
